...
{   
    ...
    for (xroof = _start_pt(0)-1; xroof<= _start_pt(0)+1; xroof +=0.1)
    {
        ...
        {
            zroof = 3.0;//fixed roof height at 3m in Vicon frame
            roofpt.x = xroof;
            roofpt.y = yroof;
            roofpt.z = zroof;
...
{   
	...
	//ROS_INFO("vertcoordx = %f,vertcoordy = %f,vertcoordz = %f",coord_x,coord_y,coord_z);
	//ROS_INFO("ptidx_0 = %d,ptidx_1 = %d,ptidx_2 = %d",pt_idx[0],pt_idx[1],pt_idx[2]);
    ...
    */           
    ...
    {   
        collide  = false; 
     //   ROS_INFO("vertex_idx(0,1) = %d, step length = %d", vertex_idx(0, 1), _step_length);
...
*/       
   ...
   // ROS_INFO("xu = %f, xl = %f,yu = %f, yl = %f,zu = %f, zl = %f",x_u, x_l, y_u, y_l, z_u, z_l);
...
{   
    ...
    if(_is_use_fm)
    {
      ...
      //  ROS_INFO("time from FMM = %f, %f, %f, %f, %f",time[0],time[time.size()-20],time[time.size()-10],time[time.size()-5],time[time.size()-1]);
    ...
    {   
       ...
       //    ROS_INFO("corridor[%d] time = %f",i,corridor[i].t);
        }
        _is_emerg = false;
        _has_traj = true;
...
{   
 ...
 //   ROS_INFO("startpt = %f, %f,%f",_start_pt[0],_start_pt[1],_start_pt[2]);
    for(int i = 1; i < (int)corridor.size(); i++){
        points.push_back(corridor[i].center);
//	ROS_INFO("center[%d] = %f, %f,%f",i,(corridor[i].center)[0],(corridor[i].center)[1],(corridor[i].center)[2]);
//	ROS_INFO("corridor[%d] time = %f",i,corridor[i].t);
	}

    points.push_back (_end_pt);
 //   ROS_INFO("endpt = %f, %f,%f",_end_pt[0],_end_pt[1],_end_pt[2]);
    ...
    for(int i = 0; i < (int)points.size() - 1; i++)
    {
        ...
        double acct = (_Vel - V0) / _Acc * ((_Vel > V0)?1:-1); //time of acceleration
        double accd = V0 * acct + (_Acc * acct * acct / 2) * ((_Vel > V0)?1:-1);//distance of acceleration
        double dcct = _Vel / _Acc; //time of deceleration                                             
        double dccd = _Acc * dcct * dcct / 2; //distance of deceleration                                  
...
//	ROS_INFO("time available = %f,acct = %f, accd = %f, dcct = %f, dccd = %f, _Acc = %f, _Vel = %f",dtxyz, acct, accd, dcct, dccd,_Acc, _Vel);
    }
